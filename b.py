...
import os
try:
    import requests, os, threading, random, time, sys, cloudscraper
    ...
    z = "python -m pip install "; os.system('%srequests' % (z)); os.system('%scolorama' % (z)); os.system('%spystyle' % (z)); os.system('%sthreading' % (z)); os.system('%sos-sys' % (z)); os.system('%sthreading' % (z)); os.system('%srandom' % (z)); os.system('%stime' % (z)); os.system('%scloudscraper' % (z))
    print(f"\n{Fore.MAGENTA}[{Fore.RESET}!{Fore.MAGENTA}] {Fore.RESET}Imports successful!")
    time.sleep(1)
...
proxyless = False
started = False
os.system(f"title Solo.to Username Checker - Starting...")
clear()
    ...
    while True:
        for line in users:
            ...
            if f"@{line}" in s:
                ...
                print(f"{Fore.YELLOW}[{Fore.RESET}!{Fore.YELLOW}] {Fore.RESET}Error ({r.status_code}): {Fore.YELLOW}" + line + f"{Fore.RESET}")
                with open ("error.txt", "a") as f:
                    f.write(line + "\n")
                    error += 1
            else:
                print(f"{Fore.YELLOW}[{Fore.RESET}!{Fore.YELLOW}] {Fore.RESET}Error ({r.status_code}): {Fore.RED}" + line + f"{Fore.RESET}")
                with open ("error.txt", "a") as f:
                    f.write(line + "\n")
                    error += 1
            if proxyless == True:
                os.system(f"title Solo.to Username Checker - Status: {count}/{len(users)} - Free: {free} - Taken: {taken} - Ratelimited: {ratelimited} - Error: {error} - Mode: Proxyless [WARNING]")
            elif proxyless == False:
                os.system(f"title Solo.to Username Checker - Status: {count}/{len(users)} - Free: {free} - Taken: {taken} - Ratelimited: {ratelimited} - Error: {error} - Using proxy: {proxyDict}")
...
try:
    if os.stat("check.txt").st_size == 0:
        ...
        os.system(f"title Solo.to Username Checker - check.txt is empty! Exiting...")
        time.sleep(1)
        sys.exit()
    ...
    if proxyless == False and not os.path.exists("proxies.txt"):
        ...
        os.system(f"title Solo.to Username Checker - proxies.txt does not exist! Exiting...")
        time.sleep(1)
        sys.exit()
        ...
        if os.stat("proxies.txt").st_size == 0:
            ...
            os.system(f"title Solo.to Username Checker - proxies.txt is empty! Exiting...")
            time.sleep(1)
            sys.exit()
        else:
    ...
    os.system(f"title Solo.to Username Checker - Exiting. If it keeps, just close the program.")
    time.sleep(1)
    exit()